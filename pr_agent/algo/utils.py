...
def get_pr_agent_footer() -> str:
    """PR-Agent attribution footer: names the model that produced the output plus the
    generation time. Provider/command agnostic — values are captured in the AI handler
    (per-command) and read here. Returns "" if disabled or model unknown."""
    if not get_settings().get("config.publish_pr_agent_footer", True):
        return ""
    model = get_settings().get("config.pr_agent_used_model", None) or get_settings().get("config.model", None)
    if not model:
        return ""
    parts = [f"✨ **PR-Agent** · Generated by `{model}`"]
    secs = get_settings().get("config.pr_agent_gen_seconds", 0) or 0
    if secs:
        parts.append(f"⏱️ {round(secs)}s")
    return "\n\n> " + " · ".join(parts)
